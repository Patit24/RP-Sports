    ...
    <div className="flex-1 p-6 md:p-10 max-w-[1600px] mx-auto space-y-8 animate-fadeIn">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div>
            {/* Mobile Enquiries Card List (Visible on mobile screens) */}
            <div className="block md:hidden divide-y divide-slate-100">
              {filteredEnquiries.map((enq) => {
                ...
                return (
                  <div key={enq.id} className="p-4 flex flex-col gap-3 hover:bg-slate-50 transition-colors">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className="font-mono font-black text-sm text-slate-900 bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-lg">
                        {enq.quantity} Jerseys
                      </span>
                    </div>

                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-1.5 text-xs">
                      <div className="flex items-center justify-between">
                        <strong className="text-slate-900 font-bold">{enq.customerName}</strong>
                        <span className="text-slate-500 font-mono text-[11px]">{enq.phone}</span>
                      </div>
                      <p className="font-bold text-slate-800 line-clamp-1">{enq.productName}</p>
                      <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-slate-200/50">
                        <span>Team: <strong>{enq.teamName || "—"}</strong></span>
                        <span className="flex items-center gap-1"><MapPin className="w-3 h-3 text-slate-400" /> {enq.deliveryCity}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <div className="flex-1">
                        <select
                          ...
                          className={`w-full text-xs font-bold px-3 py-2 rounded-xl border cursor-pointer focus:outline-none ${stConfig.bg} ${stConfig.color} ${stConfig.border}`}
                        >
                          {ALL_STATUSES.map((st) => (
                      ...
                      <a
                        ...
                        className="p-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 rounded-xl text-xs font-bold transition-colors inline-flex items-center justify-center"
                      ...
                      <button
                        ...
                        className="px-3.5 py-2 bg-[#111111] hover:bg-[#CC0000] text-white rounded-xl text-xs font-bold uppercase tracking-wider transition-colors inline-flex items-center gap-1"
                      >
                    ...
                    return (
                      <tr key={enq.id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-4 px-4 align-middle text-right space-x-2">
                          ...
                          <button
                            onClick={() => setSelectedEnquiry(enq)}
                            className="inline-flex items-center gap-1 px-2.5 py-1.5 bg-[#111111] hover:bg-[#CC0000] text-white rounded-lg text-xs font-bold transition-colors cursor-pointer"
                          >
